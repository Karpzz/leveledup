...
export class OTCProcessor {
  ...
  private async checkBalances(walletAddress: string): Promise<any> {
    ...
    const balances = await this.getTokens(walletAddress);
  ...
  async processOTCTrades() {
    ...
    try {
      const trades = await this.db.collection('otc').find({
        status: { $in: ['awaiting_token', 'awaiting_solana', 'transferring_funds'] }
      }).toArray();
      ...
      for (const trade of trades) {
        ...
        if (trade.status === 'awaiting_solana') {
          ...
          if (receivedSol >= expectedSol - tolerance) {
            ...
            console.log(`⚠️ Insufficient SOL received. Needs ${amountNeeded} more`);
            await this.db.collection('otc').updateOne(
              { _id: new ObjectId(trade._id) },
              { 
                $set: { 
        ...
        if (trade.status === 'transferring_funds') {
          ...
          try {
            ...
            const connection = new Connection(
              `${process.env.RPC_URL}?api-key=${process.env.HELIUS_API_KEY}`,
              "confirmed"
            );
            ...
            if (senderSolBalance < 0.01 * LAMPORTS_PER_SOL) {
              await this.db.collection("otc").updateOne(
                { _id: new ObjectId(trade._id) },
                { $set: { status_message: `The escrow wallet needs at least 0.01 SOL, has ${senderSolBalance / LAMPORTS_PER_SOL}`, updatedAt: new Date() }}
              );
            ...
            console.log(`✅ token token has ${mintInfo.decimals} decimals`);
            const rawAmount = Math.floor(amount * Math.pow(10, mintInfo.decimals));
            console.log(`Sending ${amount} token (${rawAmount} raw units)`);
        
            console.log(`🔎 Sender ATA for ${senderKeypair.publicKey.toBase58()}...`);
            ...
            const senderAccountInfo = await getAccount(connection, senderATA.address);
            console.log(`Sender balance: ${Number(senderAccountInfo.amount) / Math.pow(10, mintInfo.decimals)} token`);
            if (Number(senderAccountInfo.amount) < rawAmount) {
              await this.db.collection("otc").updateOne(
                { _id: new ObjectId(trade._id) },
                { $set: { status_message: `Insufficient Tokens: ${Number(senderAccountInfo.amount)} available, ${rawAmount} needed`, updatedAt: new Date() }}
              );
            ...
            const recipientAccountInfo = await getAccount(connection, recipientATA.address);
            ...
            await this.db.collection("otc").updateOne(
              { _id: new ObjectId(trade._id) },
              { $set: { status: "transferred_funds", tokenTxId: signature, updatedAt: new Date() } }
            );
